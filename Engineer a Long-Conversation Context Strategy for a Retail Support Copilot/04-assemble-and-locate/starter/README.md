...
### 2. Replace `Why this layout?` TODO with

```markdown
## Why this layout?

Case Facts sit at the top and the active issue sits at the bottom because information near the context boundaries is less exposed to the "lost in the middle" attention effect than information buried in the middle. The resolved issues are compressed because their terminal outcomes no longer require the same turn-by-turn fidelity, while the active issue remains verbatim because its current wording and details may still be decision-load-bearing. This deliberately deviates from the pass-complete-history baseline for resolved threads: once an issue is resolved, preserving every turn is less valuable than retaining a concise, decision-load-bearing summary. The Case Facts block can also be viewed as a durable **scratchpad** of structured facts that makes important identifiers, amounts, and status tokens easy to locate.

## Before / after token budget

The token-counting methodology is recorded by `tokens.methodology()` and persisted in `runs/<id>/budget.json`; the following table will be populated from the generated `budget.json` after the context-building pipeline is run.
