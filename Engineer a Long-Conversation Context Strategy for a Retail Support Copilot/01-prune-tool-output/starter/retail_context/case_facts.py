...
_SYSTEM_PROMPT = """
You extract persistent case facts from a retail customer-support transcript.

Return EXACTLY one JSON object and nothing else.

The JSON object MUST contain exactly these 12 keys:
...
- If a field is missing from the transcript, return null for that field.
- DO NOT invent, infer, or guess missing values.
- refund_amount_usd must be a JSON number, not a string.
- IDs, status tokens, and payment-method last4 values must be strings.
- Preserve snake_case status tokens exactly as they appear in the source.
- Payment-method last4 values must remain zero-padded strings.
- Output JSON only.
- Do not output prose.
...
def extract(
    ...
    *,
    ...
    raw_text, input_tokens, output_tokens = complete_with_system(
        _SYSTEM_PROMPT,
        user,
        model=model,
        max_tokens=2048,
