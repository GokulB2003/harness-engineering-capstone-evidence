...
ACTIVE_TITLES: dict[str, str] = {
    "payment": "# Active issue: Payment-method update",
}
...
def build(case_facts: CaseFacts, compressed: Compressed) -> AssembledContext:
    ...
    # IMPORTANT: active_text must remain byte-exact.
    active_block = f"{active_title}\n\n{compressed.active_text}"

    # 4. Assemble in exact top → middle → bottom order.
    markdown = (
        case_facts_block
        + "\n"
    ...
    # 5. Return the complete assembled context.
